...
function tellJoke(joke) {
	VoiceRSS.speech({
            key: 'f5bbccd0cdf04b1bb7490d4dec9e4b40',
            src: joke,
            hl: 'en-us',
            v: 'Linda',
            r: 0, 
...
const setu = document.getElementById('1');
const deliver = document.getElementById('2');
const JokeC = document.getElementById('JokeC');
...
async function getJokes() {
	...
	try {
		...
		setu.innerHTML = data.setup;
		deliver.innerHTML = data.delivery;		
			joke = `${data.setup} ... ${data.delivery}`;
			JokeC.style.visibility = "visible";
...
function enableJokeC() {
	JokeC.style.visibility = "hidden";
}
button.addEventListener('click', getJokes);
audioElement.addEventListener('ended', enableBtn);
audioElement.addEventListener('ended', enableJokeC);
	JokeC.style.visibility = "hidden";
